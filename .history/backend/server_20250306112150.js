...
// Định nghĩa route cụ thể trước middleware tĩnh
app.get('/testdauvao2', (req, res) => {
    const baseDir = __dirname; // Thư mục hiện tại của server.js (backend)
    const indexDir = path.join(baseDir, '../INDEX'); // Thư mục INDEX
    const filePath = path.join(indexDir, 'testdauvao2.html'); // Đường dẫn đến file testdauvao2.html
    console.log('Thư mục gốc server:', baseDir);
    console.log('Thư mục INDEX:', indexDir);
    console.log('Đang tìm file testdauvao2.html tại:', filePath); // Log debug
    if (fs.existsSync(filePath)) {
        ...
        res.status(404).json({ 
            ...
            filePath: filePath,
            suggestion: 'Vui lòng kiểm tra tên file hoặc đường dẫn trong thư mục INDEX.'
        });
    }
});
