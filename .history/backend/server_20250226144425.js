...
app.get('/api/news', async (req, res) => {
    try {
        console.log("🔍 Gửi yêu cầu đến NewsAPI (Top Headlines)...");
        // Thử lấy tin tức từ top-headlines
        let response = await axios.get(`https://newsapi.org/v2/top-headlines?category=education&language=vi&apiKey=${NEWS_API_KEY}`);
        let data = response.data;

        // Nếu không có tin tức (articles rỗng), thử tìm từ everything với từ khóa
        if (!data.articles || data.articles.length === 0) {
            console.log("⚠️ Không tìm thấy tin tức từ Top Headlines, thử tìm từ Everything...");
            response = await axios.get(`https://newsapi.org/v2/everything?q=education+OR+giáo+duc&language=vi&sortBy=publishedAt&apiKey=${NEWS_API_KEY}`);
            data = response.data;
            console.log("✅ Phản hồi từ NewsAPI (Everything):", data);
        } else {
            console.log("✅ Phản hồi từ NewsAPI (Top Headlines):", data);
        }
