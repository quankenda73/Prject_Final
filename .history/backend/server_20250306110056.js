...
app.get('/testdauvao2', (req, res) => {
    const filePath = path.join(__dirname, '../INDEX', 'testdauvao2.html');
    console.log('Đang tìm file tại:', filePath); // Log để debug
    if (fs.existsSync(filePath)) {
        res.sendFile(filePath);
    } else {
        console.error('File testdauvao2.html không tồn tại tại:', filePath);
        res.status(404).json({ message: 'Trang testdauvao2 không tồn tại!' });
    }
});
